...
func (r *ReuniaoPostgres) Create(ctx context.Context, reuniao *models.Reuniao) error {
	query := `INSERT INTO reunioes (id, projeto_id, data_agendada, status, observacoes, created_at, updated_at) 
	          VALUES ($1, $2, $3, $4, $5, $6, $7)`
	now := time.Now()
	_, err := r.db.ExecContext(ctx, query, reuniao.ID, reuniao.ProjetoID, reuniao.DataAgendada, reuniao.Status, reuniao.Observacoes, now, now)
	return err
}

func (r *ReuniaoPostgres) GetByID(ctx context.Context, id uuid.UUID) (*models.Reuniao, error) {
	query := `SELECT id, projeto_id, data_agendada, status, observacoes, created_at, updated_at, deleted_at 
	          FROM reunioes WHERE id = $1 AND deleted_at IS NULL`
	row := r.db.QueryRowContext(ctx, query, id)

	var reuniao models.Reuniao
	err := row.Scan(&reuniao.ID, &reuniao.ProjetoID, &reuniao.DataAgendada, &reuniao.Status, &reuniao.Observacoes, &reuniao.CreatedAt, &reuniao.UpdatedAt, &reuniao.DeletedAt)
	if err != nil {
...
func (r *ReuniaoPostgres) GetByProjeto(ctx context.Context, projetoID uuid.UUID) ([]*models.Reuniao, error) {
	...
	for rows.Next() {
		var reuniao models.Reuniao
		if err := rows.Scan(&reuniao.ID, &reuniao.ProjetoID, &reuniao.DataAgendada, &reuniao.Status, &reuniao.Observacoes, &reuniao.CreatedAt, &reuniao.UpdatedAt, &reuniao.DeletedAt); err != nil {
			return nil, err
		}
...
func (r *ReuniaoPostgres) Update(ctx context.Context, reuniao *models.Reuniao) error {
	query := `UPDATE reunioes SET projeto_id = $1, data_agendada = $2, status = $3, observacoes = $4, updated_at = $5 
	          WHERE id = $6 AND deleted_at IS NULL`
	_, err := r.db.ExecContext(ctx, query, reuniao.ProjetoID, reuniao.DataAgendada, reuniao.Status, reuniao.Observacoes, time.Now(), reuniao.ID)
	return err
}
